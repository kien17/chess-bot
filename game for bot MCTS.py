...
def main():
    ...
    running = True
    while running:
        if board.turn == BOT_COLOR and not waiting_for_promotion and not board.is_game_over():
            bot = MCTS_bot(BOT_COLOR, board.copy(), MAX_ITERATIONS)
            best_move = bot.get_best_move()
            if best_move:
                board.push(best_move)
                print(f"Bot chọn: {best_move.uci()}")
